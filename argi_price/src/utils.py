...
def process_data(raw_file, 산지공판장_file, 전국도매_file, 품목명, scaler=None):
    ...
    '깐마늘(국산)': {
        'target': lambda df: (df['품종명'] == '깐마늘(국산)') & (df['거래단위'] == '20 kg') & (df['등급'] == '상품'),
    ...
    '무': {
        'target': lambda df: (df['품종명'] == '무') & (df['거래단위'] == '20키로상자') & (df['등급'] == '상'),
...
def save_result(filename, product_predictions):
    submit_dir = "data/submissionfiles"

    sample_submission = pd.read_csv(f'./{submit_dir}/sample_submission.csv', encoding='utf-8')

    for product_name, predictions in product_predictions.items():
        sample_submission[product_name] = predictions

    if os.path.exists(f"./{submit_dir}/{filename}"):
        sample_submission.to_csv(f'./{submit_dir}/alter_{filename}', index=False, encoding='utf-8')
        print(f'Save file: alter_{filename}')
    else:
        sample_submission.to_csv(f'./{submit_dir}/{filename}', index=False, encoding='utf-8')
