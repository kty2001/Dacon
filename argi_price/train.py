...
for product_name in pbar_outer:
    ...
    for epoch in range(CFG.epoch):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'weights/{rootname}/best_model_{product_name}.pth')
        
        if epoch % 10 == 9:
            print(f'Epoch {epoch+1}/{CFG.epoch}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}')
    
    train_result[product_name] = round(best_val_loss, 5)
    print(f'Best Validation Loss for {product_name}: {best_val_loss:.4f}')
    
    ### 추론 
    product_prediction = []

    model.load_state_dict(torch.load(f'weights/{rootname}/best_model_{product_name}.pth'))
    ...
    products_predictions[product_name] = product_prediction
    pbar_outer.update(1)
