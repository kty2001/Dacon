...
from src.model import PricePredictionLSTM, PricePredictionBiLSTM, PricePredictionGRU, PricePredictionTransformer
from src.model import model_train, model_valid
from src.dataset import AgriculturePriceDataset
from src.utils import CFG, PRODUCT_LIST, process_data, save_result
...
for product_name in pbar_outer:
    ...
    for fold_num, (train_index, valid_index) in enumerate(tscv.split(dataset)):
        train_data = torch.utils.data.Subset(dataset, train_index)
        val_data = torch.utils.data.Subset(dataset, valid_index)        

        train_loader = DataLoader(train_data, CFG.batch_size, shuffle=False)
        val_loader = DataLoader(val_data, CFG.batch_size, shuffle=False)

        input_size = len(dataset.numeric_columns)
        # model = PricePredictionGRU(input_size, CFG.hidden_size, CFG.num_layers, CFG.output_size).cuda()
        model = PricePredictionTransformer(input_size, CFG.num_heads, CFG.hidden_size, CFG.num_layers, CFG.output_size).cuda()
        criterion = nn.L1Loss()
        ...
        for epoch in range(CFG.epoch):
            ...
            if val_loss < best_val_loss:
                best_val_nmae = val_loss
                torch.save(model.state_dict(), f'weights/{rootname}/best_{product_name}_tss{fold_num}.pth')
            
            if (epoch+1) % 30 == 0:
                print(f'Epoch {epoch+1}/{CFG.epoch}, Train Loss: {train_loss:.4f}, Valid Loss: {val_loss:.4f}')

        best_val_losses.append(best_val_nmae)
    
    average_val_loss = np.mean(best_val_losses, axis=0)
    train_result[product_name] = round(average_val_loss, 5)
    print(f'Mean TSS Validation Loss for {product_name}: {average_val_loss:.4f}')
    
    fold_prediction = []

    for fold_num in range(tscv.n_splits):
        product_prediction = []
        model.load_state_dict(torch.load(f'weights/{rootname}/best_{product_name}_tss{fold_num}.pth'))
        ...
        for i in pbar_inner:
            ...
            price_scaler.min_ = products_scalers[product_name].min_[price_column_index-1]
            price_scaler.scale_ = products_scalers[product_name].scale_[price_column_index-1]
...
save_result(filename=f"{rootname}.csv", product_predictions=products_predictions)
